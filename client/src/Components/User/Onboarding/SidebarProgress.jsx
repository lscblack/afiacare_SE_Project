...
const SidebarProgress = ({ currentStep, steps }) => (
  <div className="flex  justify-center gap-10 items-center ">
    {steps.map((step, index) => (
      <div key={index} className={`flex  bg-white p-4 md:p-4 rounded-md items-center mb-4 max-md:py-6 ${index <= currentStep ? "text-[#39827a] font-medium" : "text-gray-300"}`}>
        ...
        <span className="text-sm max-md:hidden ">{step}</span>
      </div>
    ))}
  </div>
);
